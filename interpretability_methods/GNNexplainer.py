from interpretability_methods.PGExplainer_RE.GNNExplainer import GNNExplainer
import numpy as np
import torch
...
def GNNexplainer(classifier_model, config, dataset_features, GNNgraph_list, current_flold=None, cuda=0, **kwargs):
    # GNNgraph_list = GNNgraph_list[:10]
    # Initialise settings
    config = config
    interpretability_config = config["interpretability_methods"]["GNNexplainer"]
    ...
    explainer = GNNExplainer(classifier_model, edges_list, features_list, task, epochs=100, lr=0.005, reg_coefs=[0.03, 0.01])
    explainer.prepare(range(len(features_list)))
    ...
    for GNNgraph in GNNgraph_list:
        ...
        if len(kwargs) > 0:
            ...
            graph, expl = explainer.explain(i, explain_class='max')
            expl = expl.detach().numpy()
            attribution = [0. for j in range(data.num_nodes)]
            # for j in range(len(expl)):  # 把每条边的重要性分到两个端点上
            ...
            graph_non, expl_non = explainer.explain(i, explain_class='min')
            expl_non = expl_non.detach().numpy()
            attribution_non = [0. for j in range(data.num_nodes)]
            # for j in range(len(expl_non)):  # 把每条边的重要性分到两个端点上
    ...
    if interpretability_config["sample_ids"] is not None:
        ...
        output_for_generating_saliency_map.update({"GNNExplainer_class_%s" % str(label): []
                                                   for _, label in
                                                   dataset_features["label_dict"].items()})
        output_for_generating_comparing_saliency_map = {}
        output_for_generating_comparing_saliency_map.update({"GNNExplainer_class_non%s" % str(label): []
        ...
        for index in graph_idxes:
            ...
            if len(output_for_generating_saliency_map["GNNExplainer_class_%s" % str(tmp_label)]) < \
                    interpretability_config["number_of_samples"]:
                output_for_generating_saliency_map["GNNExplainer_class_%s" % str(tmp_label)].append(
                    (output_for_metrics_calculation[index]['graph'], output_for_metrics_calculation[index][tmp_label]))
                output_for_generating_comparing_saliency_map["GNNExplainer_class_non%s" % str(tmp_label)].append(
