...
class PGExplainer(BaseExplainer):
    ...
    def prepare(self, indices=None, explain_class='max'):
        """
        ...
        self.train(indices=indices, explain_class=explain_class)

    def train(self, indices=None, explain_class='max'):
        """
        ...
        for e in tqdm(range(0, self.epochs)):
            ...
            for n in indices:
                ...
                if explain_class == 'max':
                    id_loss = self._loss(masked_pred, torch.argmax(original_pred).unsqueeze(0), mask, self.reg_coefs)
                elif explain_class == 'min':
                    id_loss = self._loss(masked_pred, torch.argmin(original_pred).unsqueeze(0), mask, self.reg_coefs)
                loss += id_loss
