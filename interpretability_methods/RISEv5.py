...
class rise():
    ...
    def generate_masks(self, N, p, num_of_nodes, n2n):
        """
        生成掩码
        p: 掩码为1的概率
        N: 生成掩码的数量
        ...
        n2n: 原邻接矩阵
        """
        masks = np.ones((N, num_of_nodes, num_of_nodes))  # 邻接矩阵掩码 共n张
        node_masks = (np.random.rand(N, num_of_nodes) < p).astype(float)  # 概率1-p置为0 节点掩码 共n张
        for mask, node_mask in zip(masks, node_masks):  # 对每一张掩码
            for i in range(num_of_nodes):  # 对掩码中的每一个节点
                if node_mask[i] == 0:  # 这个节点对应的边，以p的概率把它掩掉
                    nonzero_index = torch.where(n2n[i].to_dense() > 0)[0].detach().numpy()  # 找出这个节点对应的边（邻接矩阵中为1的索引）
                    random.shuffle(nonzero_index)  # 把这些边随机打乱一下，只取百分之五十使用
                    for idx in nonzero_index[:int(len(nonzero_index)/2)]:
                        mask[i, idx] = 0
        return masks, node_masks
    ...
    def attribute(self, node_feat, n2n, subg, graph, N=2000, p=0.5):
        ...
        N: 生成掩码的数量
        """
        masks, node_masks = self.generate_masks(N, p, len(node_feat), n2n)
...
def RISEv5(classifier_model, config, dataset_features, GNNgraph_list, current_flold=None, cuda=0):
    """
