...
def unserialize_pickle(dataset_name):
	...
	# Relabel nodes
	for i in range(len(nxgraph_list)):  # 对于每一张图
		relabel_mapping = {}
		j = 0
		for n in nxgraph_list[i].nodes:  # 图中的每一个节点 重新映射到从0开始
			relabel_mapping[n] = j
			j += 1

		nxgraph = nx.relabel_nodes(nxgraph_list[i], relabel_mapping, copy=True)  # 用新的节点号构成新的nxgraph

		nxgraph_list[i] = nxgraph  # 新生成的图都重新存回到nxgraph_list当中

	# Prepare graph mapping dict and node mapping dict to retain the order of the graph and node labels
	graph_mapping_list = []
	node_mapping_list = []
	for nxgraph in nxgraph_list:
		graph_mapping_list.append(nxgraph.graph['label'])  # 图对应的label存在graph['label']中

	graph_label_set = set(graph_mapping_list)  # 统计一下有几类
	i = 0
	for graph_label in sorted(graph_label_set):  # 把图的类别映射到数字上 dict['class'] = num
		graph_labels_mapping_dict[str(graph_label)] = i
		i += 1

	if "label" in nxgraph.nodes[0].keys():  # 节点有没有label 有label的话也相应映射节点的label 和图label一样
		for nxgraph in nxgraph_list:
			for node in nxgraph.nodes:
				node_mapping_list.append(nxgraph.nodes[node]['label'])
	...
	for nxgraph in nxgraph_list:
		...
		if "attribute" in nxgraph.nodes[0].keys():
			...
			node_features = np.stack(node_features, axis=0)
			node_feature_flag = True
		else:  # 如果节点本身没有特征的话，标none，后面会自动生成一些特征出来
			node_feature_flag = False
			node_features = None
...
def load_model_data(dataset_name, k_fold=5, dataset_autobalance=False, print_dataset_info=True):
	...
	:param dataset_name: name of the dataset to use 数据集的名称
	:param k_fold: the number of folds to split the dataset 要把数据集分成几个folds
	:param test_number: if specified, use this to split dataset instead of k_fold
	:param dataset_autobalance: whether to balances dataset by class distribtuion if it is too skewed.  数据集如果过于倾斜，要不要处理
	:param print_dataset_info: whether to print information on the dataset 是否要打印一下数据集的信息
	:return:
	'''
