...
class Zorro(AbstractGraphExplainer):
    ...
    def explain_node(self, node_idx, full_feature_matrix, edge_index, tau=0.15, recursion_depth=np.inf,
                     save_initial_improve=False, type="max"):
        ...
        neighbor_subset = torch.tensor(range(num_nodes))
        self.computation_graph_feature_matrix = self.full_feature_matrix
        self.computation_graph_edge_index = edge_index
        mapping = node_idx
        ...
        with torch.no_grad():
            log_logits = self.model(x=self.computation_graph_feature_matrix,
                                    edge_index=self.computation_graph_edge_index)
            self.predicted_label = log_logits.argmax(dim=-1).squeeze()
            if type == "min":
                self.predicted_label = log_logits.argmin(dim=-1).squeeze()
        ...
        if minimal_nodes_and_features_sets is None:
            minimal_nodes_and_features_sets = [(np.zeros((1, num_nodes)), np.zeros((1, self.num_features)), None)]
...
def distortion(model, node_idx=None, full_feature_matrix=None, computation_graph_feature_matrix=None,
               ...
               soft_distortion=False, detailed_mask=None, type=None
               ):
    ...
    # test
    # random_matirx = -0.2 * torch.rand((samples, num_nodes_computation_graph, num_features), generator=rng, device=device) + 0.1

    for i in range(samples):
        random_features = torch.gather(full_feature_matrix,
                                       dim=0,
                                       index=random_indices[i, :, :])  # 从原始矩阵中 随机抽取特征作为噪声 用于扰动的

        # test
        # random_features = random_matirx[i]
        ...
        if soft_distortion:
            correct += log_logits.softmax(dim=-1).squeeze()[predicted_label]
        else:
            distorted_labels = log_logits.argmax(dim=-1)
            # if type == "min":
            #     distorted_labels = log_logits.argmin(dim=-1)
            if distorted_labels.squeeze() == predicted_label:
                correct += 1
